...
let imagesLoaded = 0;
const totalImages = 11; // Updated to include all images

function imageLoaded() {
    imagesLoaded++;
    if (imagesLoaded === totalImages) {
        // All images are loaded, waiting for user interaction to start the game
        // Optionally display a loading complete message
    }
}
...
// Function to start the game (no longer called automatically)
function startGame() {
    // backgroundMusic.play(); // Moved to the user interaction event
    update();
}
...
// Add event listener for the start button (in index.html)
...
// Function to resize the canvas for mobile responsiveness
function resizeCanvas() {
    const aspectRatio = 800 / 400; // Original width / height
    ...
    // Update ground positions to match new canvas width
    groundX = 0;
    groundX2 = canvas.width;
